...
and a "TrajectoryBuffer" approach for batch updates.
...
3. For each global iteration:
   ...
   - Each worker runs a policy gradient loop with a TrajectoryBuffer:
       * Collects an entire episode (or multiple episodes) of transitions,
       * Does one batch update at the end of each episode, so the GPU sees bigger operations.
...
Disclaimer: This is STILL an illustrative example. Real trading RL is more complex.
"""
...
def log_message(msg):
    """Posts a text message to the queue for the UI to display."""
...
def load_all_csvs():
    """
    Finds files matching 'BITGET_ETHUSDT.P_*.csv', loads them, and returns
    (list_of_dataframes, list_of_timeframes).
    """
...
def merge_timeframes(dfs):
    """
    Merges a list of DataFrames on 'time' with outer join,
    forward-fills missing data, drops any remaining NaNs,
    and converts 'time' to datetime.
    """
    merged = dfs[0]
    for df in dfs[1:]:
...
def compute_indicators(merged, tfs):
    """
    Computes ATR, RSI, Bollinger Bands for each timeframe,
    storing them as new columns in 'merged'.
    """
    for tf in tfs:
        high = f'high_{tf}'
        low = f'low_{tf}'
        close = f'close_{tf}'
        merged[f'atr_{tf}'] = ta.volatility.average_true_range(
            merged[high], merged[low], merged[close], window=14, fillna=True
        )
...
class TrajectoryBuffer:
    """
    Stores an entire episode of (obs, action, log_prob, reward).
    After the episode ends, we convert everything to big tensors
    and do a single batch update on the policy.
    """
    def __init__(self):
        self.obs = []
        self.log_probs = []
    ...
    def get_tensors(self, device="cpu"):
        """
        Returns (obs_tensor, log_probs_tensor, rewards_tensor).
        Each shape: [T, ...] where T is # steps in the episode.
        """
        obs_tensor = torch.stack(self.obs).to(device)         # [steps, obs_dim]
        log_probs_tensor = torch.stack(self.log_probs).to(device)  # [steps]
        rewards_tensor = torch.tensor(self.rewards, dtype=torch.float32, device=device)
        return obs_tensor, log_probs_tensor, rewards_tensor
...
################################################################################
# WORKER FUNCTION (with TrajectoryBuffer)
################################################################################
...
def refine_worker_torch(worker_id, tf_val, config, merged_df, global_state, device="cuda"):
    ...
    for ep in range(epochs):
        ...
        # Reset environment for one episode
        obs = env.reset()
        done = False

        # Create a TrajectoryBuffer for this episode
        ...
        while not done:
            ...
            trajectory.store(obs_flat, log_prob, reward)

            obs = new_obs

        # Convert entire trajectory to big batch
        obs_tensor, log_probs_tensor, rewards_tensor = trajectory.get_tensors(device=device)

        # Compute returns for the entire episode
        returns = []
        G = 0
        for r in reversed(rewards_tensor.cpu().numpy()):
            G = r + gamma * G
            returns.insert(0, G)
        ...
        # Policy gradient update (one big batch)
        # log_probs_tensor: [T], returns: [T]
    ...
    # final simulation: deterministic run
    obs = env.reset()
    done = False
    total_reward = 0.0
    trades = 0
...
class RefineApp:
    ...
    def refinement_loop(self):
                ...
                for tf_val in tfs:
                    for w in range(worker_multiplier):
                        ...
                        worker_futures.append(
                            executor.submit(refine_worker_torch,
                                            worker_id, tf_val, self.config,
                                            self.merged_df.copy(),
                                            self.global_state, self.device)
                        )
                worker_states = []
        ...
        # Optionally do not auto-quit the UI:
...
def main():
    parser = argparse.ArgumentParser(description="Parallel RL Refinement with advanced UI (PyTorch) using real candle environment and TrajectoryBuffer.")
    ...
    # Ensure 'date' column is present if needed
