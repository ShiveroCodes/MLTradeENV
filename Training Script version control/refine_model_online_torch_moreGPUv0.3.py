#!/usr/bin/env python3
"""
RL Trading Agent with Dynamic Rewards, Short Episodes, Hard Maximum Hold, and Checkpoint Resumption

This script implements a parallel online policy gradient agent using PyTorch.
Key improvements:
  - The environment splits the dataset into shorter episodes (episode_length) so trades can only accumulate rewards for a limited number of candles.
  - A hard maximum hold (max_hold) forces trade closure after a fixed number of candles.
  - Only a fixed stop loss (SL) is enforced, so the agent must learn take profit (TP) on its own.
  - Checkpoint resumption: When the script restarts, it loads the latest iteration (e.g., iteration 10) and resumes from iteration 11.
  - Normalization is applied to all features. (Print out ATR values if needed to adjust baseline_atr.)
"""
...
class RLTradingEnv:
    ...
    Candle-by-candle trading environment that:
      ...
      - Implements a hard maximum hold by ending an episode after episode_length candles.
    ...
    def __init__(self, df, window_size=30, close_col='close_1', trade_cost=0.001,
        ...
        self.stop_loss_pct = stop_loss_pct      # e.g., 0.5%
        self.min_horizon = min_horizon
        self.max_horizon = max_horizon
        self.penalty_coef = penalty_coef
        self.random_start = random_start
        self.episode_length = episode_length     # maximum number of candles per episode
        self.reset()

    def reset(self):
        """Reset environment state and start a new episode."""
    ...
    def step(self, action):
        """
        Execute one step.
          1. Check for forced exit by stop loss.
          2. Process agent's action (1=Buy, 2=Sell, 0=Hold).
          3. Apply soft holding penalty.
          4. Force episode end if episode_length is reached.
        """
        curr_price = self.df.loc[self.index, self.close_col]
        reward = 0.0
        trade_executed = False

        # Incremental reward for holding a position
        if self.position != 0:
            reward = (curr_price - self.last_price) * self.position

        # Check for forced exit via stop loss
        forced_exit = False
        if self.position != 0:
        ...
        if forced_exit:
            ...
            # Process agent action (1=Buy, 2=Sell; 0=Hold does nothing)
        ...
        # Apply trade cost (and bonus) if trade executed
        if trade_executed:
            reward += 0.05  # bonus for executing a trade
            reward -= self.trade_cost

        self.last_price = curr_price
        self.index += 1

        # Force episode termination if episode_length reached
        if self.index >= self.start_index + self.episode_length:
            self.done = True
            if self.position != 0 and self.trade_open_index is not None:
                self._close_position(curr_price)

        # Also end if we've reached the end of the dataset
    ...
    def _close_position(self, curr_price):
        """Close the current position, record hold time, and return PnL."""
        reward = 0.0
        if self.position == 1:
...
def refine_worker_torch(worker_id, tf_val, config, merged_df, global_state, device="cuda"):
    try:
        ...
        # Create environment with short episodes and hard max hold
        env = RLTradingEnv(
            merged_df,
...
def load_latest_checkpoint_torch(base_filename="multi_tf_model.pt", prefix="multi_tf_model_refined_iter", ext=".pt"):
    ...
    return ckpt["model_state"], best_iter
...
class RefineApp:
    ...
    def init_global_policy(self):
        ...
        refined_ckpt, last_iter = load_latest_checkpoint_torch(
        ...
        if refined_ckpt is not None:
            try:
                ...
                self.current_iteration = last_iter
            except Exception as ex:
    ...
    def refinement_loop(self):
        ...
        for it in range(start_iter, total_iterations + 1):
            ...
            log_message(f"\n--- Global Iteration {it}/{total_iterations} ---")
            worker_futures = []
