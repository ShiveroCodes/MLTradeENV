#!/usr/bin/env python3
"""
RL Trading Agent with Dynamic Rewards, Shortened Episodes, and Hard Maximum Hold

This script implements a parallel online policy gradient agent using PyTorch.
Key improvements:
  - The environment now splits the dataset into shorter episodes (using episode_length)
    so that a trade can only accumulate rewards over a limited number of candles.
  - A hard maximum hold is enforced (max_hold) to force closure of trades after a set number of candles.
  - Stop loss (SL) is enforced while the agent must learn its own take profit (TP).
  - Normalization is applied to all features; please inspect ATR values to set baseline_atr appropriately.
"""
...
def compute_indicators(merged, tfs):
    for tf in tfs:
        ...
        # Compute RSI and Bollinger Bands (if needed)
...
class RLTradingEnv:
    ...
    def __init__(self, df, window_size=30, close_col='close_1', trade_cost=0.001,
        ...
        self.stop_loss_pct = stop_loss_pct          # Fixed stop loss, e.g., 0.5%
    ...
    def reset(self):
        ...
        if self.random_start:
            # Ensure we have enough candles to complete an episode and hold period.
    ...
    def step(self, action):
        ...
        4. Force episode termination if episode_length reached.
        """
    ...
    def _close_position(self, curr_price):
        """Close the current position and record the hold time. Return PnL."""
        reward = 0.0
        if self.position == 1:
    ...
    def get_final_profit(self):
        """Return total accumulated reward (profit percentage)."""
        return self.total_reward if hasattr(self, 'total_reward') else None
...
def refine_worker_torch(worker_id, tf_val, config, merged_df, global_state, device="cuda"):
    try:
        ...
        # Create environment with shortened episodes and hard max hold
        env = RLTradingEnv(
            merged_df,
...
class RefineApp:
    ...
    def refinement_loop(self):
        ...
        for it in range(1, iterations + 1):
                    ...
                    for w in range(worker_multiplier):
                        ...
                        worker_futures.append(
                            executor.submit(
                                ...
                                self.merged_df.copy(),# copy of data
            ...
            # Average worker states to update global model
            new_state = {}
...
def main():
    parser = argparse.ArgumentParser(description="RL Trading with Dynamic Rewards, Short Episodes & Hard Max Hold")
