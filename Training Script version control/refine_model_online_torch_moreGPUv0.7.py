...
Key improvements (v0.7):
  ...
  - After a configurable iteration (e.g. 150), losing workers are discarded entirely.
  - UI changes: Removed worker progress bars. Added a "Stop after current iter" button and a net profit bar that shows the average net profit range over the past N iterations.
"""
...
class RefineUI:
    def __init__(self, root):
        ...
        # We'll store the latest history for drawing the bar
        self.net_profit_history = []
        self.lookback = 10  # default; will be updated from config later

        # Treeview for worker simulation results
        ...
        # Remove worker progress bars (not needed)
        self.worker_rows = {}

        # Stop callback (to be set by RefineApp)
    ...
    def update_net_profit_bar(self, history, current):
        # history: list of last N iteration profit percentages
        # current: current iteration profit percent
        if not history:
            return
        # Compute symmetric range based on max absolute value in history
        min_val = min(history)
        max_val = max(history)
        X = max(abs(min_val), abs(max_val))
        if X == 0:
            X = 10  # default range if no variation
        # Clear canvas
        ...
        # Draw left and right boundary labels
        ...
        # Map current value to a position on the bar
        pos = 10 + ((current + X) / (2 * X)) * (width - 20)
        # Determine color: red if negative, green if positive, yellow if near zero
        if current < 0:
            color = "red"
        elif current > 0:
            color = "green"
    ...
    def poll_queue(self):
        while not log_queue.empty():
            ...
            if msg_type == "log":
                # Append to text log
                print(data)  # or update a log widget if desired
                ...
                # Data contains 'history' and 'current'
...
def refine_worker_torch(worker_id, tf_val, config, merged_df, global_state, device="cuda"):
    try:
        ...
        for b in range(num_batches):
            ...
            optimizer.step()
            scheduler.step()
            # (Worker progress updates removed)
...
################################################################################
# MAIN PARALLEL REFINEMENT APP WITH STOP SIGNAL & PROFIT HISTORY
################################################################################
...
class RefineApp:
    def __init__(self, ui, config, merged_df, device):
        ...
        self.iter_profit_history = []  # store average net profit per global iteration
    ...
    def refinement_loop(self):
        ...
        for it in range(start_iter, iterations + 1):
            ...
            all_profits = [sim.get("total_profit_pct", 0.0) for sim in sim_results]
            if it > discard_iter:
                filtered = [(st, p) for st, p in zip(worker_states, all_profits) if p > 0]
                if not filtered:
                    log_message(f"No profitable workers at iteration {it}. Skipping update.")
                    continue
                worker_states = [x[0] for x in filtered]
                profits = [x[1] for x in filtered]
            ...
            # Compute average net profit for this iteration (arithmetic mean of sim_results)
            valid_profits = [p for p in all_profits if p is not None]
            if valid_profits:
                iter_profit = sum(valid_profits) / len(valid_profits)
            else:
                iter_profit = 0.0
            self.iter_profit_history.append(iter_profit)
            # Send update to UI (only last 'lookback' iterations)
            history_slice = self.iter_profit_history[-lookback:]
            ui_post_message("net_profit_update", {"history": history_slice, "current": iter_profit, "lookback": lookback})

            # Check for stop signal after current iteration is complete
