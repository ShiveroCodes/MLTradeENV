...
and Realized Profit Bonus

This script implements a parallel online policy gradient agent using PyTorch.
Key improvements:
  - The environment splits the dataset into shorter episodes (using episode_length)
    so that a trade can only accumulate rewards over a limited number of candles.
  - A hard maximum hold (episode_length) forces trade closure after a set number of candles.
  - A fixed stop loss (SL) is enforced while the agent must learn its own take profit (TP).
  - Realized profit on trade closure is rewarded: if a trade is profitable, a bonus is added
    based on a fixed reward plus a multiplier per % gain.
...
################################################################################
# MODULAR RL TRADING ENVIRONMENT WITH SHORT EPISODES & HARD MAX HOLD
################################################################################
...
class RLTradingEnv:
    ...
    Candle-by-candle trading environment that:
      ...
      - Implements a hard maximum hold (episode_length) so that each episode ends after a fixed number of candles.
    ...
    def __init__(self, df, window_size=30, close_col='close_1', trade_cost=0.001,
        ...
        self.stop_loss_pct = stop_loss_pct          # e.g., 0.5% stop loss
        self.min_horizon = min_horizon
        self.max_horizon = max_horizon
        self.penalty_coef = penalty_coef
        self.random_start = random_start
        self.episode_length = episode_length         # Maximum candles per episode
        self.trade_open_reward = trade_open_reward     # Bonus for executing/closing trade
        self.profit_trade_multi = profit_trade_multi   # Multiplier per percent profit

        self.reset()

    def reset(self):
        """Reset the environment state and define a fixed episode length."""
    ...
    def step(self, action):
        """
        Execute one step.
         1. Check forced exit by SL.
         2. Execute trade logic based on agent action.
         3. Apply hold penalty.
         4. Force episode termination if episode_length is reached.
        """
        ...
        if forced_exit:
            ...
            # Process agent's action (1=Buy, 2=Sell; 0=Hold)
            reward += self._execute_trade(curr_price, action=action)

        # Apply soft holding penalty if position is open
        ...
        # Apply trade cost and bonus if a trade was executed
        if trade_executed:
    ...
    def _calculate_hold_penalty(self, hold_duration):
        """Apply a soft penalty if hold_duration exceeds the dynamic horizon."""
    ...
    def _execute_trade(self, curr_price, action=None):
        """
        Handle the agent's trade actions:
          - Action 1: Buy (open long)
          - Action 2: Sell (open short)
          - Action 0: Hold (do nothing)
        Closes any existing position if switching sides.
        Returns the reward from trade execution.
        """
        reward = 0.0
        if action == 1:  # Buy
            if self.position != 1:
    ...
    def _close_position(self, curr_price):
        """
        Close the current position, record the hold time, and return PnL.
        If the trade is profitable, add a bonus reward based on the profit percentage.
        """
        reward = 0.0
        if self.position == 1:
        ...
        # If profit is realized, add bonus reward: fixed bonus + multiplier * profit_pct
        if profit_pct > 0:
    ...
    def get_avg_hold_time(self):
        """Return average hold time (in candles) for closed trades."""
        return sum(self.hold_times) / len(self.hold_times) if self.hold_times else 0.0

    def get_final_profit(self):
        """Optional: Return total accumulated reward (if implemented)."""
        return None
...
def main():
    parser = argparse.ArgumentParser(description="RL Trading with Dynamic Rewards, Short Episodes, Hard Max Hold & Checkpoint Resumption")
