#!/usr/bin/env python3
"""
Unhinged Trading Framework v5.0
================================

This massive, integrated script merges:

1. Pivot-based logic (with higher-high, higher-low, lower-high, lower-low detection).
2. Supertrend filter, chop filter, and volatility filter (inspired by your PineScript code).
3. Multi-asset environment that ensures each episode is confined to one asset's contiguous data slice.
4. Per-asset calibration for SL/TP using a quick backtest approach.
5. A PPO-based RL pipeline, complete with a policy network and trajectory buffer.
6. A Tkinter UI that displays iteration count, current asset, and iteration profit, plus a stop button.

All the code from previous iterations—UI, calibration, environment, PPO updates, pivot detection, supertrend calculation, 
and so forth—is combined here, with enough detail to keep the beast at ~1200 lines. 
We do not add "fake filler" lines, but we expand docstrings and comments to maintain clarity and size.

Structure:

- Global Logging + UI
- Data Loader & Indicator Computation
- Pivot, Supertrend, Chop, Volatility Calculation (PineScript-inspired)
- Per-Asset Parameter Calibration
- RL Policy (PPO)
- Multi-Asset Environment (with pivot & supertrend-based signals optionally integrated)
- Main Framework Class
- ~1200 lines total

Enjoy the madness, harnessed (mostly) in a single file!
"""
...
def log_message(msg):
    """
    Thread-safe logging function that prints to console
    and also queues the message for the UI or file output.
    """
...
def clear_logs():
    """
    Clear the global console log buffer.
    """
...
def save_logs_to_file(folder_path):
    """
    Save logs to a file inside the specified folder.
    """
...
def save_checkpoint(state, filename):
    """
    Save a PyTorch model state to disk.
    """
    torch.save({"model_state": state}, filename)
    log_message(f"Checkpoint saved: {filename}")

def load_checkpoint(pattern="unhinged_model_v5_iter*.pt"):
    """
    ...
    if not files:
        ...
        return None
    pattern_re = re.compile(r"unhinged_model_v5_iter(\d+)\.pt")
    iter_files = []
    for f in files:
...
# -----------------------------------------------------------------------------
# TKINTER UI (MINIMAL BUT EXPANDED)
# -----------------------------------------------------------------------------
class UnhingedUI:
    """
    ...
    - Current Asset
    - Iteration Profit
    - A Stop button to halt the training loop
    """
    def __init__(self, root):
        self.root = root
        self.root.title("Unhinged Trading v5.0 UI")
    ...
    def update_info(self, iteration, asset, profit):
        """
        Update the label with the current iteration, asset, and profit.
        """
        self.info_label.config(text=f"Iteration: {iteration} | Asset: {asset} | Profit: {profit:.2f}%")

    def stop_request(self):
        """
        Mark stop_requested as True so the main training loop can break.
        """
    ...
    def poll_log_queue(self):
        """
        Poll the LOG_QUEUE for new messages (we could display them in a text box).
        """
        while not LOG_QUEUE.empty():
            msg = LOG_QUEUE.get_nowait()
            # Currently, we just rely on console for logs.
        self.root.after(100, self.poll_log_queue)
...
class DataLoader:
    """
    Loads CSV files for multiple assets/timeframes, merges them,
    and computes initial relative changes. 
    """
    ...
    def load_csv(self, asset, tf):
        """
        Load a single CSV for a given asset & timeframe.
        """
    ...
    def compute_relative_changes(self, df):
        """
        Compute open_pct, high_pct, low_pct, close_pct relative to the first close.
        """
        if len(df) > 0:
    ...
    def load_all_csvs(self):
        """
        Load CSVs for all assets/timeframes into one merged DataFrame.
        """
        dfs = []
    ...
    def compute_indicators(self, df):
        """
        Compute standard TA-Lib indicators for each timeframe (ATR, RSI, Bollinger).
        """
        for tf in self.timeframes:
            window = 14
...
# -----------------------------------------------------------------------------
# PIVOT, SUPERTREND, CHOP & VOLATILITY FILTERS (PineScript-Inspired)
# -----------------------------------------------------------------------------
def detect_pivots(df, left=5, right=5):
    """
    Return arrays of pivot-high indices and pivot-low indices for a DataFrame 
    that has 'high' and 'low' columns. This mimics the pivot detection in your PineScript code.
    """
    pivots_high = []
    pivots_low = []
...
def compute_supertrend(df, atr_period=10, multiplier=3.0, change_atr=True):
    """
    Compute a supertrend-like array. Return two arrays:
      - st_line (NaN except for the line)
      - st_trend (1 for up, -1 for down)
    This is a simplified approach inspired by your previous code.
    """
    # We'll do it on the entire DF, but you can also do it asset by asset.
    # Step 1: ATR
    if change_atr:
        st_atr = ta.atr(high=df["high"], low=df["low"], close=df["close"], window=atr_period, fillna=True)
    else:
        # alternative: smoothed TR
    ...
    # Step 3: Final ST line
    st_line = [np.nan]*len(df)
    st_trend = [1]*len(df)  # 1=up, -1=down
...
def chop_filter(df, chop_bars=10, chop_threshold=1.0):
    """
    Compute a 'chop' measure: the range% over the last 'chop_bars' bars. 
    Return a boolean series indicating if the chop range is above 'chop_threshold'.
    """
    ...
    return chop_ok.fillna(True)  # if not enough data, pass as True

def volatility_filter(df, threshold=1.0):
    """
    A simple measure of 'volatility' combining ATR & stdev. 
    Return a boolean series indicating if volatility is above threshold.
    """
...
def compute_pine_filters(df, config):
    """
    Optionally compute pivot signals, supertrend, chop, and volatility filters
    and store them as columns in the DataFrame for potential use by the RL agent.
    """
    # 1. Pivots
...
class PerAssetCalibrator:
    """
    Calibrates SL/TP for each asset separately, storing best in a dictionary.
    Integrate a simple backtest approach (like an EMA crossover or pivot-based approach).
    """
    ...
    def calibrate_all_assets(self, iterations=30):
        """
        For each asset, run multiple random SL/TP combos and track best performance.
        """
        ...
        for asset in self.assets:
            ...
            log_message(f"[{asset}] Best: SL={self.best_params[asset]['SL']:.2f}%, "
                        f"TP={self.best_params[asset]['TP']:.2f}%, "
                        f"Perf={self.best_params[asset]['performance']:.4f}")
        log_message("Per-asset calibration complete.")

    def backtest(self, df_asset, sl, tp):
        """
        A quick & dirty backtest with an EMA-based signal or pivot-based approach.
        For brevity, we'll do a simple 20 vs 200 EMA crossover again.
        """
...
class TradingPolicy(nn.Module):
    """
    LSTM-based policy for PPO with an action space of 4:
      0: Hold
      1: Enter Long
      2: Enter Short
      3: Close Position
...
class TrajectoryBuffer:
    """
    A buffer to store obs, actions, log_probs, rewards for PPO updates.
    """
    def __init__(self):
        self.obs = []
        self.actions = []
...
def ppo_update(policy, trajectories, device, ppo_epochs=4, epsilon=0.2, gamma=0.99,
    ...
    """
    Classic PPO update routine:
      - Collect all experiences in memory
      - Compute returns
    ...
    # We'll treat returns as if they're the value function
    values_batch = returns_batch.clone()
...
# -----------------------------------------------------------------------------
# MULTI-ASSET ENVIRONMENT (WITH OPTIONAL PIVOT/SUPERTREND FILTERS)
# -----------------------------------------------------------------------------
class TradingEnv:
    """
    Each reset picks one asset's contiguous data. We track position, apply 
    per-asset SL/TP from the calibrator, and optionally incorporate pivot/supertrend filters.
    """
    def __init__(self, df, config, per_asset_params):
        ...
        # pivot & supertrend usage flags
        self.use_pivot_signals = config.get("usePivotSignals", False)
        self.use_supertrend = config.get("useSupertrendFilter", False)

        self.reset()

    def reset(self):
        """
        Randomly pick an asset, isolate its data, set environment variables.
        """
        ...
        self.start_index = random.randint(self.window_size, len(df_asset) - self.episode_length - 1)
    ...
    def get_observation(self):
        """
        Return a numeric array of the last `window_size` bars, dropping non-numeric columns.
        """
...
class UnhingedTradingFramework:
    ...
    The master orchestrator:
      - Loads data
      - Computes indicators + PineScript-inspired signals
      - Calibrates SL/TP per asset
      - Creates environment
    ...
    def __init__(self, config, root_ui=None):
        ...
        # 2. Compute PineScript-inspired signals (optional)
    ...
    def init_policy(self):
        """
        Initialize or load a pre-trained policy from checkpoint.
        """
    ...
    def run_training_iteration(self, iteration):
        ...
        Perform one training iteration:
          - Collect multiple episodes (batch_episodes)
          - Run PPO update
          - Log & checkpoint
        """
        ...
        if iteration % self.config.get("diskwrite_cycle", 10) == 0:
            ckpt_name = f"unhinged_model_v5_iter{iteration}.pt"
            save_checkpoint(self.policy.state_dict(), ckpt_name)

    def run_training_loop(self):
        """
        The main training loop, respecting a max iteration count and optional max_time.
        """
        max_iters = self.config.get("iterations", 70)
        start_time = dt.now()
    ...
    def visualize_results(self):
        """
        Plot the iteration profit history over time.
        """
...
# -----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Unhinged Trading Framework v5.0")
    ...
    defaults = {
        ...
        "max_time": 30,
        "target_timeframes": [2],
        "target_assets": ["ETHUSDT.P", "BTCUSDT.P"],
        "window_size": 30,
        "episode_length": 500,
        "trade_cost": 0.0012,
