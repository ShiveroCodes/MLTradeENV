#!/usr/bin/env python3
"""
RL Trading Agent 2.4 – Multi-Asset with Fixed Asset Per Worker, Configurable Checkpointing, 
Enhanced UI & Holding Time Reward Bonus

Key changes (v2.4):
  • Checkpoints now save only every 'diskwrite_cycle' iterations as set in the config.
  • The UI now displays a fixed line per worker (avoiding endless scrolling) and updates in place.
  • Plotting of trading styles is wrapped in an exception handler to disable itself upon pyimage errors.
  • Trades now receive a bonus based on holding time so longer holds are rewarded.
"""
...
################################################################################
# RL TRADING ENVIRONMENT (with holding time bonus)
################################################################################
...
class RLTradingEnv:
    def __init__(self, df_all, window_size=30, close_col='close', trade_cost=0.001,
                 ...
                 holding_bonus_coef=0.01):  # NEW: holding bonus coefficient
        ...
        self.holding_bonus_coef = holding_bonus_coef  # NEW: store the bonus parameter
        ...
        if forced_exit:
            ...
            if self.position == 0:
                ...
                if action == 3:  # exit
                    ...
                    reward += ((curr_price - self.last_price) / self.last_price) * 100 * self.position
    ...
    def _close_position(self, curr_price):
        ...
        # NEW: Incorporate holding time bonus.
        held = self.index - self.trade_open_index if self.trade_open_index is not None else 1
        if held > 1:
            bonus_multiplier = 1 + self.holding_bonus_coef * (held - 1)
            net_trade_pct *= bonus_multiplier
...
################################################################################
# PLOT TRADING STYLES (PNG/ subfolder) – See plotting section above.
################################################################################
...
def refine_worker_torch(worker_id, tf_val, config, merged_df, global_state, device="cuda"):
    try:
        ...
        env = RLTradingEnv(
            ...
            holding_bonus_coef=config.get("holding_bonus_coef", 0.01)  # NEW: pass bonus parameter
        )
...
class RefineApp:
    ...
    def refinement_loop(self):
        ...
        for it in range(start_iter, iterations+1):
            ...
            # NEW: Only save checkpoint based on 'diskwrite_cycle' value from config.
...
def main():
    ...
    config.setdefault("diskwrite_cycle", 100)  # NEW: Set default disk write cycle
    config.setdefault("holding_bonus_coef", 0.01)  # NEW: Set default holding bonus
